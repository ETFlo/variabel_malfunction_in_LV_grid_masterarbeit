...
learning_config = {
    ...
    "number of epochs": 1,  # Florian Liszt - vorher waren es 20 Epochen
...
type = 'EV'  # PV, EV
dataset_available = True  # set to False to recreate instances from raw data # Florian Liszt (Ursprung war False, damit bei Änderung der Dataset Settings die Rohdaten neu erstellt werden; jetzt auf True gesetzt, damit nicht bei jeder Änderung der Dataset Settings die Rohdaten neu erstellt werden))
...
add_data = False  # raw_data_available = False has to be set for this! set add_data = True to add more data to raw data; # Florian Liszt (Ursprung war True, damit bei Änderung der Dataset Settings mehr Rohdaten erstellt werden; jetzt auf False gesetzt, damit nicht bei jeder Änderung der Dataset Settings mehr Rohdaten erstellt werden))
add_noise = False
...
# Simulation settings
sim_length = 14  # simulation length in days (has to be equal or bigger than sample length
