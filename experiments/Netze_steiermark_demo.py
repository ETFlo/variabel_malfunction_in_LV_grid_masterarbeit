...
data_path_stmk = os.path.join(os.getcwd(), raw_data_folder, 'stmk_sim_data')
...
learning_config = {
    ...
    'setup_chosen' : {'stmk': 'Gleinz'}, # 'Neudau', 'Gleinz', #{"phase1": {"A", "B"}, "phase2": {}},  # either 'all' for phase 1 setup A+B and phase 2 setup A+B or enter setups to be ommited such as: if phase 1, setup A should be omitted enter {"phase1": {"A"}, "phase2": {}} to omit the entire phase 1 enter {"phase1": {"A", "B"}, "phase2": {}}, if only class 'inversed' should be omitted for setup A enter {"phase1": {"A": ["inversed"]}, "phase2": {}}
...
load_estimation_training_data_available = False #create input for training of NN for load estimation
...
plot_data = False
use_case = 'stmk' # 'DSM', 'q_control', 'stmk'
...
data_path_dict = {'phase1' : data_path, 'phase2': data_path_DSM, 'stmk' : data_path_stmk}
...
if use_case == 'DSM':
    ...
    test_bays = ['NAP']
    data_path = os.path.join(data_path_stmk, f'{learning_config["setup_chosen"]["stmk"]}_PV_monitoring')
else:
    test_bays = ['B1', 'F1', 'F2']
...
elif extended and not list(learning_config['setup_chosen'].keys())[0] == 'stmk':
...
elif list(learning_config['setup_chosen'].keys())[0] == 'stmk': #also determines which cases simulated!!
    setups = {'Neudau': ['correct', 'wrong', 'inversed', 'flat', 'as_is'], 'Gleinz': ['correct', 'wrong', 'inversed', 'flat', 'as_is']}#{'Neudau': ['correct', 'wrong', 'inversed', 'flat', 'as_is'], 'Gleinz': ['correct', 'wrong', 'inversed', 'flat', 'as_is']}
else:
...
else: pf_file = 'defined differently'
pf_file_dict = {'stmk': '_PV_monitoring','phase1': 'PNDC_ERIGrid_phase1', 'phase2': 'PNDC_ERIGrid_phase2'}
resolution = '1T' #resolution of load/generation profiles to be used
t_start = None  # default(None): times inferred from profiles in data
t_end = None
step_size = 1
step_unit = 1 #0...seconds, 1... minutes
