...
class Transformer_detection:
    ...
    # Florian Liszt: Habe die Zeilen auskommentiert, da sonst die Ausführung nicht funktioniert
    ...
    def load_data(self, scenario=None, sampling=None, data_source=None, phase_info=None, grid_setup=None, marker='-'):
        ...
        elif isinstance(learning_config['setup_chosen'], dict) and list(learning_config['setup_chosen'].keys())[0] == 'stmk':
        ...
        if scenario:
            ...
            if data_source == 'simulation':
                ...
                if isinstance(learning_config['setup_chosen'], dict) and list(learning_config['setup_chosen'].keys())[0] == 'stmk':
                    for measurement in measurements:
    ...
    def detection(self):
        ...
        if self.data_mode == 'measurement_wise':
            ...
            variable_selection = self.find_most_common_PCs(
                results_pca)  # , number_of_variables = 15)   # returns dict keyed by bay name; most important variables for PCA per measurement point #returns the most important variables for the PCA per measurement point > use to to do PCA and then SVM
            if self.plot_data:
        ...
        if self.data_mode == 'combined_data':
            ...
            elif isinstance(learning_config['setup_chosen'], dict) and list(learning_config['setup_chosen'].keys())[0] == 'stmk': #list(learning_config['setup_chosen'].keys())[0] == 'stmk':
                trafo_point = 'NAP_PV_' + learning_config['setup_chosen']['stmk']
                classes = config.setups[learning_config['setup_chosen']['stmk']]
                classes = classes.remove('as_is')
                variables = ['mean voltage p.u.', 'P', 'Q']
            else: trafo_point = self.setup_chosen.split('_')[2] # Florian -> else: trafo_point = 'F2'
            ...
            if isinstance(learning_config['setup_chosen'], dict) and list(learning_config['setup_chosen'].keys())[0] == 'stmk': #list(learning_config['setup_chosen'].keys())[0] == 'stmk':
                data = create_dataset(type='combined', data=data, variables=variables,
                                      ...
                                      bay=trafo_point,
                                      labelling='stmk')
            # Florian Liszt: Anpassen der variables!!!
            # Florian Liszt: Dynamisches Laden aus detection_method_settings.py
                ...
                data = create_dataset(type='combined', data=data, variables=dynamische_variablen, name=self.setup_chosen,
